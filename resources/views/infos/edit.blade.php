        ...
        <div class="page-title">
            <div class="row">
                ...
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Edit Info</h3>
                    <p class="text-subtitle text-muted">Perbarui informasi atau pengumuman.</p>
                </div>

                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('infos.index') }}">Infos</a></li>
                    ...
                    <form action="{{ route('infos.update', $info->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="message" id="message" class="form-control @error('message') is-invalid @enderror" rows="6"
                                required>{{ old('message', $info->message) }}</textarea>
                            @error('message')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="date" id="date" value="{{ old('date', $info->date) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror"
                                required>
                                <option value="active" {{ old('status', $info->status) == 'active' ? 'selected' : '' }}>
                                    Active</option>
                                <option value="inactive"
                                    {{ old('status', $info->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-save"></i> Perbarui Info
                            </button>
                        </div>
